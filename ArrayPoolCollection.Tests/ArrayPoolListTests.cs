...
public class ArrayPoolListTests
{
    ...
    public void Huge()
    {
        ...
        Assert.Equal(CollectionHelper.ArrayMaxLength, list.Capacity);
        Assert.Equal(CollectionHelper.ArrayMaxLength, list.Count);
        ...
        Assert.Throws<OutOfMemoryException>(() => list.AddRange([-1]));

        Assert.Equal(CollectionHelper.ArrayMaxLength, list.AsReadOnly().Count);

        Assert.Equal(CollectionHelper.ArrayMaxLength, ArrayPoolList<int>.AsSpan(list).Length);

        Assert.Equal(123, list.BinarySearch(123));
        Assert.True(list.Contains(123));
        using var doubleList = list.ConvertAll(i => (double)i);
        Assert.Equal(CollectionHelper.ArrayMaxLength, doubleList.Count);

        var buffer = new int[CollectionHelper.ArrayMaxLength];
        list.CopyTo(buffer);

        Assert.Equal(CollectionHelper.ArrayMaxLength, list.EnsureCapacity(1));

        Assert.False(list.Exists(i => i == -1));
        Assert.Equal(123, list.Find(i => i == 123));
        using var evens = list.FindAll(i => i % 2 == 0);

        Assert.Equal(123, list.FindIndex(i => i == 123));
        Assert.Equal(123, list.FindLast(i => i == 123));
        Assert.Equal(123, list.FindLastIndex(i => i == 123));

        list.ForEach(i => { });
        foreach (var elem in list) { }

        using var partial = list.GetRange(123, 456);
        Assert.Equal(456, partial.Count);

        Assert.Equal(123, list.IndexOf(123));

        Assert.Throws<OutOfMemoryException>(() => list.Insert(1, -1));
        Assert.Throws<OutOfMemoryException>(() => list.InsertRange(1, Enumerable.Range(0, 1)));
        Assert.Throws<OutOfMemoryException>(() => list.InsertRange(1, [1]));

        Assert.Equal(123, list.LastIndexOf(123));
        ...
        Assert.Equal(333, list[123..456].Count);
