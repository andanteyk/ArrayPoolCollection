...
namespace ArrayPoolCollection
{
    public sealed class ArrayPoolHashSet<T> : ICollection<T>, IReadOnlyCollection<T>, ISet<T>, IReadOnlySet<T>, IDisposable
    {
        ...
        private void Resize(int newCapacity)
        {
            ...
            m_Values = SlimArrayPool<T>.Shared.Rent(newCapacity);
            ...
            if (oldValues != null)
            {
                SlimArrayPool<T>.Shared.Return(oldValues, RuntimeHelpers.IsReferenceOrContainsReferences<T>());
            }
            if (oldMetadata != null)
            {
        ...
        private void RemoveAt(int metadataIndex)
        {
            ...
            if (valueIndex != m_Size - 1)
            {
                ...
                int movingMetadataIndex = HashCodeToMetadataIndex(movingHashCode, values.Length);
        ...
        public ArrayPoolHashSet(IEnumerable<T> source, IEqualityComparer<T>? comparer)
        {
            if (source is ArrayPoolHashSet<T> cloneSource && cloneSource.m_Comparer == comparer)
            {
                ...
                m_Values = SlimArrayPool<T>.Shared.Rent(cloneSource.m_Values.Length);
        ...
        public void Dispose()
        {
            ...
            {
                SlimArrayPool<T>.Shared.Return(m_Values, RuntimeHelpers.IsReferenceOrContainsReferences<T>());
                m_Values = null;
            }
